...
class TimeTrackerApp:
    ...
    def timer_ui(self):
        # OVERVIEW FRAME
        self.overview_frame = ttk.Frame(self.root)
        # NORMAL VIEW
    ...
    def edit_current_session(self):
        if self.current_session_index is None:
            return
        self.edit_session(self.current_session_index)
    ...
    def edit_session(self, index):
        ...
        for program in session.get(
            ...
            program_list.insert(
                ...
                program)
        # Program Buttons
        program_buttons = ttk.Frame(dialog)
        ...
        for program in programs:
            try:
                if not os.path.exists(program):
                    failed_programs.append(f"{program}\n(File not found)")
                    continue
    ...
    def update_session_label(self):
        ...
        if session:
            ...
            self.session_label.config(text="No Session")
    ...
    def change_target_dialog(self):
        ...
        ttk.Label(
            dialog,
            text="Window title to track:").pack(pady=(10, 0))
    ...
    def save_time_to_csv(self):
        ...
        with FileLock(LOCK_PATH):
            ...
            # Falls heute bereits existiert
                ...
                today_row[session_name] = (format_seconds(total_seconds)
                )
            # Spaltenreihenfolge
            ordered_fields = (
                ["Date"]
            ...
            def parse_date(row):
                try:
                    return datetime.datetime.strptime(row.get("Date", ""),
                        "%d.%m.%Y"
                    )
                except ValueError:
